...
fn send_file_wifi(ui_handle: slint::Weak<AppWindow>, ip: String, port: u32, file_path: &str) {
    ...
    if let Ok(mut stream) = TcpStream::connect(&addr) {
        ...
        if let Ok(mut file) = File::open(file_path) {
            ...
            let file_size = file.metadata().unwrap().len() as f32;
            ...
            loop {
                let bytes_read = match file.read(&mut buffer) {
                    ...
                    Err(e) => {
                        ...
                        let _ = ui_handle.upgrade_in_event_loop(move |ui| {
                            ...
                            ui.set_transfer_status(error_msg.into());
                        });
                        return;
                    }
                };
                ...
                sent += bytes_read;
                let progress = (sent as f32 / file_size).min(0.99);
                let _ = ui_handle.upgrade_in_event_loop(move |ui| ui.set_sending_progress(progress));
            }
            let _ = ui_handle.upgrade_in_event_loop(move |ui| {
                ui.set_sending_file(false);
                ui.set_sending_progress(0.0);
                ui.set_transfer_status(format!("Sent {} bytes successfully", sent).into());
            });
                ...
                *file_accepted.lock().unwrap() = None;
                ...
                std::thread::spawn(move || {
                    let _ = Notification::new()
                        ...
                        .body(&format!("Incoming file: {}. Please accept or reject in the app.", filename_clone))
                        .icon(&icon_path)
                        .image_path(&icon_path)
                        .show();
                });
...
pub(crate) async fn wifi(mdns: ServiceDaemon, ui_handle: slint::Weak<AppWindow>, file_accepted: std::sync::Arc<std::sync::Mutex<Option<bool>>>) {
    ...
    let host_name_reg = host_name.clone();
    let _ = ui_handle.upgrade_in_event_loop(move |ui| ui.set_transfer_status(format!("Service registered: {}", host_name_reg).into()));
    ...
    async_std::task::spawn(async move {
        while let Ok(event) = receiver.recv() {
            if let ServiceEvent::ServiceResolved(resolved) = event {
                if resolved.get_hostname() == host_name {
                    continue;
                }
